...
| crop_slither | 0.8455 | 0.458 | 0.081 | 0.542 | -0.057 |
| crop_slither_cot | 0.8049 | 0.375 | 0.152 | 0.625 | -0.041 |
| crop_slither_multi | 0.8049 | 0.542 | 0.111 | 0.458 | 0.000 |

> DeepSeek-chat, T=0, repeat=3 均值, 41 样本（33 正 / 8 负）。边际变化为相对前一层 profile 的差值。

**结果解读**：

- **crop_only 边际增益最大（+0.033）**：代码裁剪是本次消融实验中唯一正向贡献的模块。消除无关代码使模型注意力集中于风险点，漏报率压至 0.010。**但代价显著**：裁剪同时剥离了 ReentrancyGuard 安全上下文，FPR 从 0.375 升至 0.458——fixed 变体识别率仅 0.542。这一矛盾直接驱动了后续的 Guard 注入实验。
- **crop_slither 退化（-0.057）**：Slither 的静态分析摘要以结构化文本注入，反而干扰了模型对 Solidity 代码的直接理解。
- **crop_slither_cot 进一步退化（-0.041）**：CoT 分步推理无法补偿模型对 modifier 执行语义（`_` 占位符）的理解缺失，强制推理步骤引入额外噪声。
- **crop_slither_multi 触底（Acc=0.8049, FPR=0.542）**：更多的依赖文件暴露更多外部调用模式，模型倾向于过度判定漏洞。

**核心结论**：裁剪是唯一有效的上下文优化手段，但需要更精准的裁剪策略——既保留风险模式，又不丢失安全保护信息。**这直接驱动了 slice_v1 切片引擎的设计。**
...
| 方案 | Accuracy | FPR | FNR | fixed Acc | 与 crop_only 对比 |
|---|---|---|---|---|---|
| crop_only（消融最优） | 0.9024 | 0.458 | 0.010 | 0.542 | 基准 |
| slice_v1 + Guard | 0.9024 | 0.375 | 0.030 | 0.625 | FPR↓, FNR↑ |

**结果解读**：Acc 持平消融最优（0.9024），FPR 从 0.458 降至 0.375——Guard 注入让 fixed 变体识别率从 0.542 升至 0.625。**但fixed 变体仍有 100%（03_modifier_fixed 和 05_cross_contract_fixed 全误报）**——模型"看到"了 nonReentrant 源码却不理解其语义，仍然仅凭 `call{value:...}` 模式判定漏洞。**这驱动了 Prompt 规则注入实验。**
...
|---|---|---|---|---|
| slice_v1 + Guard（基线） | 0.9024 | 0.375 | 0.030 | 0.625 |
| + 解释性注释 | 0.9024 | 0.375 | 0.030 | 0.625 |
| + Prompt 规则 | 0.8699 | **0.250** | 0.101 | **0.750** |
| crop_slither + Prompt | 0.8780 | 0.292 | 0.081 | 0.708 |

**结果解读**：

- **解释性注释完全无效**：FPR 和 FNR 均未变化。模型将代码注释视为"可忽略的文档"，不作为决策约束。**这验证了 LLM 存在两条权重不对称的信息通道——指令通道（系统 Prompt 规则，优先级高）和内容通道（代码与注释，优先级低）。**
- **Prompt 规则效果显著但矫枉过正**：FPR 从 0.375 大幅降至 0.250，fixed Acc 从 0.625 升至 0.750。**但 FNR 从 0.030 飙升至 0.101**——模型学会了"看到 nonReentrant → 判 safe"的短路策略，错误地将部分存在绕过路径的 insecure 样本也判定为安全。**这驱动了切片级绕过检测。**
...
- **FNR 从 0.101 回落至 0.071（降幅 30%）**：安全摘要使模型能区分"真正安全"（`[SAFE]`）和"有锁但可绕过"（`[BYPASS-RISK]`），不再一刀切放过所有带 nonReentrant 的样本。
- **Acc 升至 0.878（slice_v1 系列最高）**：虽然 FPR 从 0.250 回升至 0.333——部分 fixed 变体的安全摘要被误标 `[BYPASS-RISK]`——但 FNR 的显著改善超过了 FPR 的回升，净 Acc 为正增长。
- **验证了"证据优于规则"的设计原则**：代码结构中的具体证据比纯文本约束更有效，即使模型缺乏完全的 Solidity 语义理解。

**FPR 回升的根因**：当前安全摘要的判定阈值较为激进（只要存在一个未加锁函数即报 BYPASS-RISK），导致部分实际上安全的 fixed 变体也获此标签。后续可通过调优阈值（如未加锁函数数 ≥ 2 才报风险）进一步压制 FPR。
...
| Slither 摘要 | 消融叠加 | -0.057 | 0 | +0.071 | 无 | Acc 退化 |
...
**设计启示**：每一项有效的优化都伴随着可度量的代价——FPR 与 FNR 呈跷跷板关系。最优方案取决于应用场景：若人工复核资源有限，选 FPR 最优的 slice_v1+Prompt（FPR=0.250）；若漏报成本极高，选 FNR 最低的 crop_only（FNR=0.010）或 slice_v1+bypass（FNR=0.071）。
