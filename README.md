...
> DeepSeek-chat, T=0, repeat=3 均值, 41 样本。\*标注行使用启发式回退（Slither 因 solc 版本不匹配静默失败）。修复后真实 Slither 报告结果见 4.4 节。
...
- **crop_slither 退化（-0.057）**：初版实验使用启发式回退分析（Slither 因 solc 版本不匹配静默失败），结构化摘要反而干扰模型。**修复 solc 版本匹配后，Slither 真实报告生效，crop_slither 的指标表现与下文 Slither 增强实验结果一致。**
